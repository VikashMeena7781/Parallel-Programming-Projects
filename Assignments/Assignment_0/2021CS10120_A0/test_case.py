...
if __name__ == "__main__":
    ...
    for sz in L:
        # for j in range(3):
        result, time_duration = execute_test_case(5, sz, sz, sz, "./input_path/", "./output_path/")
        if not result:
            print(f"Test case failed")
            exit
        print(f"Permutation : {5} size : {sz} time : {time_duration} seconds")
            # if result:
            #     # print(f"Test Case passed for permutation {i}", time_duration)
            #     print("Test Case passed", time_duration)
            # else:
            #     print("Test Case failed", time_duration)
            # # break
